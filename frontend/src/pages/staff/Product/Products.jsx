                          ...
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              <div className="relative">
                                {product.image ? (
                                  <img src={`${product.image.startsWith('http') ? '' : import.meta.env.VITE_STORAGE_URL + '/'}${product.image}`} alt={product.name} className="w-12 h-12 rounded-xl object-contain shadow-md group-hover:scale-105 transition-transform duration-200 shrink-0" />
                                ) : (
                  ...
                  {filtered.map((product) => (
                    <div key={product.id} className="bg-white border border-gray-200 rounded-2xl p-5 hover:shadow-lg transition-all duration-300">
                      <div className="flex items-start gap-4 mb-4">
                        {product.image ? (
                          <img src={`${product.image.startsWith('http') ? '' : import.meta.env.VITE_STORAGE_URL + '/'}${product.image}`} alt={product.name} className="w-16 h-16 rounded-xl object-contain shadow-md shrink-0" />
                        ) : (
          ...
          <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto animate-slideUp" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-6">
              <div className="flex items-center gap-5">
                {viewProduct.image ? (
                  <img src={`${viewProduct.image.startsWith('http') ? '' : import.meta.env.VITE_STORAGE_URL + '/'}${viewProduct.image}`} alt={viewProduct.name} className="w-24 h-24 rounded-2xl object-contain shadow-lg shrink-0" />
                ) : (
