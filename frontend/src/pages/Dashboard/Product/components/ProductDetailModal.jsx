    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl shadow-2xl shadow-slate-500/20 max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col border border-slate-200" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="bg-slate-50 border-t border-slate-200 px-8 py-5 flex flex-col sm:flex-row items-center justify-end gap-3 shrink-0">
          <button onClick={onClose} className="w-full sm:w-auto bg-white border border-slate-200 text-slate-700 px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-slate-50 transition-all duration-200 shadow-sm">Close</button>
          <button onClick={() => { const p = product; onClose(); onEdit(p) }} className="w-full sm:w-auto bg-gradient-to-r from-teal-600 to-teal-500 text-white px-8 py-2.5 rounded-xl text-sm font-semibold hover:shadow-lg hover:-translate-y-0.5 shadow-md transition-all duration-300">Edit Product</button>
        </div>
      </div>
    </div>
  )
