  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white rounded-3xl shadow-2xl shadow-slate-500/20 max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col border border-slate-200" onClick={(e) => e.stopPropagation()}>
        <div className="bg-gradient-to-r from-teal-900 to-teal-700 px-8 py-5 flex items-center justify-between z-10 shrink-0">
        ...
        <div className="p-8 space-y-6 overflow-y-auto flex-1">
          ...
          {product.description && product.description.trim() !== '' ? (
            <div className="bg-gradient-to-r from-teal-50 to-emerald-50 rounded-xl p-5 border border-slate-200">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {product.addons?.length > 0 && (
              <div className="bg-teal-50/50 rounded-xl p-4 border border-slate-200">
                <div className="text-[10px] font-bold text-teal-600 uppercase tracking-wider mb-2.5">Addons</div>
                <div className="flex flex-wrap gap-2">
                  {product.addons.map((addon) => (
                    <span key={addon.id} className="bg-white text-teal-700 border border-slate-200 text-xs font-semibold px-3 py-1.5 rounded-lg shadow-sm">{addon.name}</span>
                  ))}
                </div>
              </div>
            )}
              ...
              <div className="bg-teal-50/50 rounded-xl p-4 border border-slate-200">
                ...
                <div className="flex flex-wrap gap-2">
                  {product.sugar_levels.map((level) => (
                    <span key={level.id} className="bg-white text-teal-700 border border-slate-200 text-xs font-semibold px-3 py-1.5 rounded-lg shadow-sm">{level.name}</span>
                  ))}
                </div>
              </div>
            )}
        ...
        <div className="bg-slate-50 border-t border-slate-200 px-8 py-5 flex flex-col sm:flex-row items-center justify-end gap-3 shrink-0">
          <button onClick={onClose} className="w-full sm:w-auto bg-white border border-slate-200 text-slate-700 px-6 py-2.5 rounded-xl text-sm font-semibold hover:bg-slate-100 transition-all duration-200 shadow-sm">Close</button>
          <button onClick={() => { const p = product; onClose(); onEdit(p) }} className="w-full sm:w-auto bg-gradient-to-r from-teal-900 to-teal-700 text-white px-8 py-2.5 rounded-xl text-sm font-semibold hover:shadow-lg hover:-translate-y-0.5 shadow-md transition-all duration-300">Edit Product</button>
        </div>
      </div>
    </div>
  )
