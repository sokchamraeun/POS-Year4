  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={onClose}>
        <div
          className="bg-white rounded-2xl shadow-xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-hidden border border-teal-200/60"
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header */}
          <div className="sticky top-0 z-10 bg-white border-b border-teal-100 px-6 py-4 flex items-center justify-between">
            ...
            <button onClick={onClose} className="p-2 hover:bg-teal-50 rounded-lg transition-colors">
            ...
            <form onSubmit={handleSubmit} encType="multipart/form-data" className="p-6 space-y-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  <div className="w-20 h-20 rounded-xl bg-gray-50 border border-teal-200 flex items-center justify-center overflow-hidden shrink-0">
                    {form.imagePreview ? (
                      <img src={form.imagePreview} alt="Preview" className="w-full h-full object-cover" />
                    ) : (
                      <svg className="w-7 h-7 text-teal-300" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0 0 22.5 18.75V5.25A2.25 2.25 0 0 0 20.25 3H3.75A2.25 2.25 0 0 0 1.5 5.25v13.5A2.25 2.25 0 0 0 3.75 21Z" />
                      </svg>
                    )}
                  </div>
                  <div>
                    <label className="cursor-pointer inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-white border border-teal-200 hover:border-teal-300 hover:bg-teal-50 transition-colors text-sm font-medium text-teal-600">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-teal-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400 transition-colors placeholder:text-gray-300"
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full border border-teal-200 rounded-lg px-3.5 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400 transition-colors"
                  >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full border border-teal-200 rounded-lg px-3.5 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400 transition-colors resize-none placeholder:text-gray-300"
              ...
              {/* Status Toggle */}
              <div className="flex items-center justify-between bg-teal-50/50 rounded-xl px-4 py-3 border border-teal-100">
                <div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {allSizes.map(size => {
                    ...
                    return (
                      <div key={size.id} className={`flex items-center gap-3 px-3.5 py-2.5 rounded-lg border transition-colors ${isActive ? 'border-teal-300 bg-teal-50/40' : 'border-teal-100 bg-white'}`}>
                        <button
                          type="button"
                          onClick={() => handleSizeToggle(size.id)}
                          className={`w-[18px] h-[18px] rounded border-[1.5px] flex items-center justify-center shrink-0 transition-colors ${isActive ? 'bg-teal-500 border-teal-500' : 'border-teal-300 bg-white'}`}
                        >
                          {isActive && (
                        ...
                        <input
                          ...
                          className="w-28 border border-teal-200 rounded-lg px-3 py-1.5 text-sm text-right focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400 transition-colors"
                        />
                      </div>
                    )
                  })}
                    ...
                    return (
                      <button
                        ...
                        className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm border transition-colors ${isActive
                          ...
                          : 'bg-white text-gray-600 border-teal-200 hover:border-teal-300'}`}
                      >
                        {addon.name}
              ...
              <div className="flex gap-3">
                ...
                <button
                  type="button"
                  onClick={onClose}
                  className="px-5 py-2.5 rounded-lg text-sm font-medium text-gray-600 border border-teal-200 hover:bg-teal-50 transition-colors"
                >
                  Cancel
                </button>
              </div>
      ...
      {cropOpen && cropImage && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl m-4 overflow-hidden border border-teal-200/60">
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input type="number" min={1} value={outputWidth} onChange={(e) => setOutputWidth(e.target.value)} placeholder="Width" className="w-full border border-teal-200 rounded-lg px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400 transition-colors" />
                  <span className="text-gray-400 text-sm">×</span>
                  <input type="number" min={1} value={outputHeight} onChange={(e) => setOutputHeight(e.target.value)} placeholder="Height" className="w-full border border-teal-200 rounded-lg px-3.5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400 transition-colors" />
                  <span className="text-xs text-gray-400">px</span>
                </div>
              </div>
              <div className="flex gap-3 pt-1">
                <button onClick={handleCropCancel} className="flex-1 px-4 py-2.5 rounded-lg text-sm font-medium text-gray-600 border border-teal-200 hover:bg-teal-50 transition-colors">Cancel</button>
