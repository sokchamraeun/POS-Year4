...
export default function SugarLevel() {
  ...
  const openCreate = () => { setEditing(null); setForm({ name: '', requires_input: false }); setShowModal(true) }

  const openEdit = (item) => { setEditing(item); setForm({ name: item.name, requires_input: !!item.requires_input }); setShowModal(true) }
              ...
              <label className="mb-5 flex items-start gap-3 rounded-xl border-2 border-slate-200 p-3 cursor-pointer hover:border-amber-300 transition-colors">
                ...
                <span>
                  ...
                  <span className="block text-xs text-slate-400 mt-0.5">
                    When selected during an order, the customer/staff can type an exact amount (e.g. Extra Sugar).
                  </span>
                </span>
              </label>
