  ...
  return (
    <>
      <div className="mb-6 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <MetricCard label="Staff Members" value={rows.length} tone="blue" icon={Users} />
        <MetricCard label="Total Orders" value={totalOrders} tone="teal" icon={ShoppingCart} />
        <MetricCard label="Total Sales" value={money(totalRevenue)} tone="green" icon={DollarSign} />
      </div>
            ...
            <table className="min-w-[820px] w-full text-sm">
              <thead className="bg-teal-800 text-white">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right font-black">Discount Given</th>
                  <th className="px-4 py-3 text-right font-black">Refunds</th>
                </tr>
              </thead>
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={row.id || index} className="border-t border-slate-100 hover:bg-teal-50/50">
                    <td className="px-4 py-3 font-black text-slate-800">{row.name || 'Unknown'}</td>
                    <td className="px-4 py-3 text-slate-600 capitalize">{row.role || '—'}</td>
                    <td className="px-4 py-3 text-right text-slate-700">{row.orders_count ?? 0}</td>
                    <td className="px-4 py-3 text-right font-black text-slate-900">{money(row.revenue)}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{money(row.total_discount)}</td>
                    <td className="px-4 py-3 text-right text-slate-600">{row.refund_orders ?? 0}</td>
                  </tr>
                ))}
              </tbody>
            </table>
