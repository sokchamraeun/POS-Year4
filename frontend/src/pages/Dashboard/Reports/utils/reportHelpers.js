...
export function buildPrintHtml({ title, subtitle, rows, summary, saleUserLabel }) {
  const tableRows = rows.length ? rows : [{ Message: 'No data available' }]
  const columns = Object.keys(tableRows[0])
  const generatedAt = new Date().toLocaleString()
  const generatedBy = getCurrentUserName()
      ...
      <head>
        ...
        <style>
          * {
          ...
          /* For 7 boxes:
            row 1 = 4 boxes
            row 2 = 3 boxes full width */
          .summary-card.second-row-3 {
      ...
      <body>
        <div class="header">
          ...
          <div class="shop">
            ...
            POS Cafe Report<br />
            Generated: ${safeText(generatedAt)}<br />
            Sale by: ${safeText(saleUserLabel || 'All Users')}<br />
            Generated by: ${safeText(generatedBy)}
          </div>
        </div>
